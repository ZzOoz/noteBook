...
git log  // 查看日志版本改变
git log –pretty=oneline // 查看日志版本改變（精簡模式：只会显示版本号和提交时的备注信息）
...
#### **github提交创建多个本地分支并且推送远程**

```javascript
// 在本地要先提交文件 即git commit之后才能看到分支
git checkout -b origin // 创建分支
git add . 
git commit -m "xxxx"  // 提交到暂存区
git remote add origin github仓库 // 关联
git push origin origin // 推送远程
```
...
```javascript
git status // 查看刚刚git add 提交的文件是否提交错了
...
```javascript
1、使用git bash输入命令touch .gitignore 新建文件
2、在这个文件里面输入node_modules 表示过滤这个文件夹里面的文件
3、提交远程分支
```
...
## 小程序的currentTarget和Target的区别是什么？

currentTarget指向事件所绑定的元素，而target始终指向事件发生时的元素

参考：https://blog.csdn.net/qq_36901488/article/details/82378619
...
通过：event.currentTarget.dataset获取自定义数据
...
**将接口promise化可以进行链式调用，不会有回调嵌套的麻烦**
...
# :birthday::birthday:小程序多功能实现整合:birthday::birthday:

**声明：具体实现功能在github可查看**：https://github.com/ZzOoz/miniProgram-all-in-one

### 1、消息向左拖拉删除功能

思路：根据touchStart里面的event.touches.clientX和touchEnd里面的event.changedTouches.clientX进行比较，得出的结果用变量left插入数据itemData的某一项的left属性中，在wxml中绑定style的left属性进行偏移，而中间的触摸过程touchMove也是如此不断更新left变量进行拖拽效果的更新
...
# **:cloud::cloud:**小程序云开发**:cloud:**:cloud:

## 云函数中的event与context

event对象和context对象。其中event对象指的是SDK触发云函数时传入的事件；而context对象则包含此调用的调用信息和函数的运行状态。采用不同的调用方式以及调用链条，返回的event对象和event对象的值是不一样的。
...
### 1、建议在云函数的exports.main函数之外只定义常量或者公共方法，不要定义变量。main函数之外声明的变量可能会被缓存，而不是每次都执行：
...
### 2、云数据库表在新建的时候需要勾选权限所有用户可读，仅创建者可读写这个选项，不然在获取数据云数据的时候获取不到



**3、在操作云数据的时候，小程序端和云函数端是不一样的，小程序端是用``const db = wx.cloud.database()``，云函数端是```const db = cloud.database()。``**



**4、查询操作符和更新操作符的使用位置：查询操作符是用来构建筛选记录的条件，所以要写在where内；而更新操作符，则是对记录的字段进行字段级别的写操作，要写在update内**



**5、使用where条件所筛选的始终都是记录，而update则是对筛选返回的记录列表进行字段级别的更新**
...
**4.1、查询中国GDP在3000亿元以上的前10个城市，并要求不显示_id字段，显示城市名、所在省份以及GDP，并按照GDP大小降序排列。**
...
参考：https://juejin.cn/book/6897486502482149376/section/6897525958589284355
...
参考：https://juejin.cn/book/6897486502482149376/section/6897526288308535310
...
| 更新操作符之字段操作符 |                                      |        |                                  |
| :--------------------: | ------------------------------------ | ------ | -------------------------------- |
|          set           | 设置字段为指定值，也可以新增一个字段 | remove | 删除字段                         |
|         rename         | 重命名字段的名称                     |        |                                  |
|          inc           | 原子操作，自增字段值                 | mul    | 原子操作，自乘字段值             |
|          min           | 如果字段值小于给定值，设为给定值     | max    | 如果字段值大于给定值，设为给定值 |



|      查询操作符之数组      |                                                |           |                                            |
| :------------------------: | ---------------------------------------------- | --------- | ------------------------------------------ |
|            all             | 数组所有元素是否满足给定条件                   | elemMatch | 数组中包含至少一个满足给定的所有条件的元素 |
|            size            | 数组长度是否等于给定值                         |           |                                            |
| **更新操作符之数组操作符** |                                                |           |                                            |
|            push            | 往数组尾部增加元素（可以指定位置）             | pop       | 从数组尾部删除一个元素                     |
|           shift            | 将数组头部元素删除                             | unshift   | 往数组头部增加元素                         |
|          addToSet          | 原子操作，如果不存在给定元素则添加元素         | pull      | 根据查询条件删除数组中的元素               |
|          pullAll           | 和pull类似，不过只能指定常量值，可以被pull取代 |           |                                            |
...
//数组里的值也可以是数字，比如取值为10、20这两个值，写法为_.in([10,20])，这里不要错误理解为10到20。

// 跨字段逻辑操作符
.where(
    // 查询gdp大于3000 居住人口大于500或者建成面积大于300的数据
    // gdp: _.gt(3000)和resident_pop:_.gt(500)是逻辑与，而与 builtup_area: _.gt(300)}的关系是逻辑或。 _.or([{条件一},{条件二}])内是一个数组，条件一与条件二又构成逻辑与的关系。
  {
    gdp: _.gt(3000),       
...
//exists字段：比如每个记录都有字段_id，可以通过如下选中所有记录
.where({
  _id:_.exists(true)
})

//exists字段：比如我们可以判断字段是否存在，从而进行有选择的更新，比如记录不存在gdp这个字段时，可以将字段的值清洗为null
.where({
  gdp:_.exists(false)
})
.update({
...
//  elemMatch匹配和財經一樣的tag標簽，push更新標簽數組
db.collection('user').where({
...
// 匹配tags數組的第二個元素為徒步愛好者的數據
// 可以使用“字段名.数组的index”的方式獲取數據
.update({
  data:{
    "tags.1":"徒步爱好者"
  }
...
//匹配和更新对象里面的元素
//company的值是一个对象，在匹配对象的条件时，我们可以使用点表示法（更加推荐这种方法）和传入相同结构的对象的方式，比如我们要筛选企业的名称是“腾讯”，岗位是“前端开发工程师”的用户：
//点表示法，更推荐这种方式，
.where({
  "company.name":"腾讯",
...
//$匹配第一个更改数据 注意：$不能用于更新嵌套在两层及两层以上的数组里的字段。
db.collection('user')
.where({
...
操作集合里面的记录有get、add、remove、update方法，这些方法也都是基于数据库集合的引用Collection的。

##### 10.1、在小程序端新增记录只能一条条添加
...
##### 10.3、删除和更新记录（这里如果不配置自定义安全规则那么在小程序端是无法删除和更新的，而云函数端则是拥有最高权限可以执行删除和更新批量操作）
...
**基于集合记录引用Document的四个请求方法：获取单个记录数据get、删除单个记录remove、更新单个记录update、替换更新单个记录set**

##### 1、注意：和Collection不同（操作集合中的记录），前者的增删改查是可以批量多条的，而基于Document则是操作单条记录，前者可以替代后者的操作
...
//下面是基于Collection的替代写法，openid指的是当前用户的openid
...
**云数据Collection的权限控制有两种：一种是简易权限控制，另外一种是自定义安全规则**

如果你使用的是简易权限控制，在小程序端基于Document的请求，同样也是自带了`where({_openid:"当前用户id"})`的条件（服务端没有），而简易权限控制，最多是所有用户可读，但是写操作（set、update、remove）则只能创建者可写，也就是当前用户的openid与记录_openid的值相同时才能对该记录进行写操作，也就是无法在小程序端对记录进行跨用户的写操作。**（set、update、remove需要在云函数端进行操作）**
...
**换言之：简易权限控制就是小程序提供的四种控制权限，安全规则就是自己定义的规则**

**注意：对于update这些操作需要管理员才能操作，换句话说就是在小程序端是不能进行这些操作需要在云函数端操作**
...
下面的案例，我们把用户上传网盘的数据存储到两个集合里，下面是存储用户信息的集合user，其中file是一个数组，是用户存储的文件id：
...
聚合阶段还有随机从文档中选取指定数量的记录的sample、根据给定条件和边界划分成不同组的bucket和bucketAuto，以及添加新字段的addFields等等



## 云存储的基础操作

参考：https://juejin.cn/book/6897486502482149376/section/6898595519280381966
    ...
    success: function (res) {
      ...
      wx.cloud.uploadFile({
        ...
        success: res => {
          ...
          that.setData({
            // 将上传云存储成功的fileID传到imgUrl上面去，image、video这些标签支持fileID作为src地址
            imgurl:res.fileID
          })
        },
        fail:err=>{
...
### 3、云函数端上传图片到云存储（注意：在云函数上传的图片是没有上传者的openId的）
...
exports.main = async (event, context) => {
    // 创建stream 在云函数端通过cloud.uploadFile上传文件流
...
### 4.获取文件在云存储的https链接（fileID是云存储与数据库的纽带）

小程序端调用`wx.cloud.getTempFileURL` 方法，只需要传入文件的fileID，就可以换取云存储空间指定文件的https链接，这个https链接我们可以在浏览器里打开，也可以做为图床来使用：
...
## javaScript和nodeJs和ES的区别

**JavaScript：使用了ECMAScript的语法规范，外加WebApi，缺一不可（JS包含DOM操作、BOM操作、事件绑定、Ajax等，两者结合共同完成浏览器的操作）**

**ECMAScript：定义了语法规范，写js和nodeJs都必须遵守**

**NodeJs：使用了ECMAScript的语法规范，外加nodeApi，缺一不可（node包含处理文件、处理http等，两者结合共同完成服务端的操作）**
...
2、考虑内存和CPU（优化和扩展）：**使用stream写日志记录、使用redis存session**
...
**异步IO、事件与回调函数（事件驱动）、单线程、跨平台**



## node事件机制的实现

node中大部分的模块都是继承自Event模块



### IO里面的阻塞和非阻塞、同步和异步

**在实际效果中阻塞和同步、非阻塞和异步看起来是一样的效果，但是在计算机内核是两个不同的内容**

**阻塞和非阻塞：**阻塞模式会造成应用程序的等待，知道IO完成，非阻塞反之

**同步和异步：**如果是阻塞IO调用，那么应用程序等待调用完成的过程就是一种同步状况，相反如果是非阻塞那么就是一种异步状况
...
![图片](https://mmbiz.qpic.cn/mmbiz_jpg/4o22OFcmzHmk3U6Yn0clWiaW89bOk03az6CPPp4jezBE0zFGMwfIRapteKq6AmIwSwbVSTwEloPBzibkd93ia2D6w/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
**介绍：它是一种运算受限的线性表。限定仅在表尾进行插入和删除操作的线性表；先进后出的特点**

· Push 推送：在堆栈顶部插入一个元素。

· Pop 弹出：删除最上面的元素并返回。

![图片](https://mmbiz.qpic.cn/mmbiz_jpg/4o22OFcmzHmk3U6Yn0clWiaW89bOk03az1memNKTsnRZmiceicKLPcDfIzbMezO8TPWs4CnwGPzfuSj6cuOMAUUIQ/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_jpg/4o22OFcmzHmk3U6Yn0clWiaW89bOk03azbBWSftlE4I4JVwU6gONgKgqeTeWYpHeFu86deja2N2Z4gu8w4I03pg/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_jpg/4o22OFcmzHmk3U6Yn0clWiaW89bOk03azHkQZh0JLUm5ibxuMb21DIjg4xMbgWicBqgVntJ4mjHyia3lHHrdpyfCSw/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_jpg/4o22OFcmzHmk3U6Yn0clWiaW89bOk03az187rFWycOia2LWxiaJiaAWAIA3eu9fSia6UdHdtyDPgv1ZJ7N3HHRprreA/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_jpg/4o22OFcmzHmk3U6Yn0clWiaW89bOk03azM5DlWibtnhWeoUosuaZoKlFM51JWK0US3Uhp59TtIpSjyjpwtKYniaWQ/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
### 1、vue-roll-up:基于Vue 2.x的 <垂直循环滚动跑马灯> 组件

npm地址：https://www.npmjs.com/package/vue-roll-up
...
文档说明：http://www.ptbird.cn/vue-draggable-dragging.html



### 4、vue模糊查询搜索框（基于element-ui）

组件名称：el-autocomplete

文档：https://element.eleme.cn/#/zh-CN/component/input（带输入建议一栏位置）
