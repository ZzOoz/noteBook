...
```js
// 这是一个demo.ts文件 在html中直接导入demo.ts文件和jq，但是使用$的时候不会是被因为jq时使用js写的
const teacher:string = '12'
console.log(teacher)

// 当我们在ts文件中去使用jquery文件的时候，因为jquery是基于js编写的，不知道ts，
// 所以对于这种情况就需要使用d.ts文件来描述ts给js翻译
// 当我们定义了d.ts文件时就$下面就不会飘红了
$(function(){
    alert(123)
})  // 对应declare function $ (readFunc:() => void) : void

$(function(){
    $('body').html("<div>123</div>");// 对应declare function $(selector:string):JqueryInstance
    new $.fn.init()  // 对应对象的描述（d.ts文件的第3点）declare namespace $ {namespace fn{class init{}}
    }
})
```
...
// 1.对于两个相同函数名的描述定义，这是函数的重载
// 第一个是代表参数是一个函数，这个函数返回空，然后这个外面这个函数也是返回空
declare function $ (readFunc:() => void) : void
interface JqueryInstance{
    html:(html:string)=>JqueryInstance
}
// 第二个是指参数是字符串,返回的是一个函数，这个函数的参数是字符串返回对象
declare function $(selector:string):JqueryInstance


// 如何对对象进行类型定义，以及对类进行类型定义，以及命名空间的嵌套
// 2.当$是一个对象是可以使用namespace来进行描述定义，如果嵌套就是用namespace的嵌套
declare namespace $ {
    namespace fn{
        class init{}
    }
